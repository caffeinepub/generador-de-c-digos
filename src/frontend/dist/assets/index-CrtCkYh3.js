...
${lines.join("\n\n")}`
  ...
  if (msg.includes("historial") || msg.includes("registro") || msg.includes("impresion") || msg.includes("impresión") || msg.includes("registre") || msg.includes("registré")) {
    if (historial.length === 0) {
      return {
    ...
    const last3 = historial.slice(0, 3).map(
      (e, i) => `• #${i + 1}: ${e.fecha}${e.hora ? ` ${e.hora}` : ""} — ${e.tipo === "sin-codigo" ? `${e.totalImagenes} placas${e.nombre ? ` (${e.nombre})` : ""}` : `${e.codigoInicio} → ${e.codigoFin} (${e.totalImagenes})`}`
    ).join("\n");
    return {
...
const WELCOME_MSG = "¡Hola! Soy tu asistente de producción 🤖. Puedes preguntarme cosas como: ¿por qué placa me quedé?, ¿qué pedidos tengo activos?, ¿cuánto hice hoy? También puedo generar PDFs y añadir pedidos enviados si me lo pides.";
...
function App() {
  ...
  function handleAddOrder(nombre, total, tipo, codigoInicio, codigoFin) {
    setOrders((prev) => [
      ...prev,
      {
        id: Date.now().toString(),
