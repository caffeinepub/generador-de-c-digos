...
function parseOneSegment(text) {
  ...
  let tipo = null;
  if (t.includes("matriculas") || t.includes("matricula") || t.includes("con codigo") || t.includes("con barras") || t.includes("barcode")) {
    tipo = "con-codigo";
  } else if (t.includes("placas ce") || t.includes("placa ce") || t.includes("sin codigo") || t.includes("placas") || t.includes("placa")) {
    tipo = "sin-codigo";
  ...
  let provider = null;
  const withoutCount = text.replace(/^(ultimas?\s+\d+|las?\s+\d+|los?\s+\d+|\d+)\s*/i, "").replace(/^(ultimas?|las?|los?)\s+/i, "").trim();
...
function generateFilteredPdf(segmentResults, summaryLabel) {
...
Pulsa el botón para confirmarlo.`,
  ...
  const hasFilterKeyword = msg.includes("placa") || msg.includes("matricula") || msg.includes("advertencia") || msg.includes("proveedor") || msg.includes("ultimas") || msg.includes("últimas") || /\d+\s+(placa|matricula|advertencia)/.test(msg);
  if (hasPdfKeyword && hasFilterKeyword) {
    ...
    if (segmentResults.length > 0) {
      ...
      for (const r of segmentResults) {
        ...
        const limitLabel = r.segment.limit ? `las ${r.segment.limit} últimas ` : "";
    ...
    try {
      const toSave = messages.map((m) => {
        ...
        if (((_a3 = m.action) == null ? void 0 : _a3.type) === "pdf" && m.action.handler === "filtered") {
          const { filteredEntries: _fe, ...rest } = m.action;
